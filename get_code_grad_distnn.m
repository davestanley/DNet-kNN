function [err new_code_grad] = get_code_grad_distnn (Y, triples, NNs)
...
% margin_violations = \sum_ijk n_ij (1 - y_ik) (1 - label(i,k))(1 + d_ij - d_ik)
  ...
  err = sum(marginViolations(violationIndices)) + sum(Dist_nn);
  kk2 = size(triples, 1)/(numcases*size(NNs,2));
kk1 = size(NNs,2);
new_code_grad1 = zeros(numcases, dim);
new_code_grad2 = zeros(numcases, dim);
new_code_grad3 = zeros(numcases, dim);
new_code_grad4 = zeros(numcases, dim);
...
for tmpind = 1:length(unique_ks)
 ...
 % derivative dist_nn
  for tmpind =1:numcases
    new_code_grad4(tmpind, :) = new_code_grad4(tmpind, :) + 2*(kk1*Y(tmpind,:) - sum(Y(NNs(tmpind,:),:), 1));
    for tmpind2=1:kk1
     new_code_grad4(NNs(tmpind,tmpind2),:) = new_code_grad4(NNs(tmpind,tmpind2),:) - 2* (Y(tmpind,:) - Y(NNs(tmpind,tmpind2),:));
    end
  end
new_code_grad = new_code_grad1 + new_code_grad2 + new_code_grad3;
new_code_grad = new_code_grad + kk2*new_code_grad4;
clear Dist*  margin*  new_code_grad1  new_code_grad2  new_code_grad3 ...
    unique* violation* new_code_grad4;
